...
try{
  ...
  assert.equal(await page.locator('iframe:visible').count(),0);
  assert.equal(await page.locator('.battle-settings input[type=checkbox]').count(),6);
  const preview=page.locator('[data-module="battle_preview"]'),timer=page.locator('[data-module="battle_timer_freeze"]');
  await preview.getByLabel('预览模式',{exact:true}).selectOption('imperfect');
  await timer.getByLabel('启用此模块',{exact:true}).check();
  assert.equal(await preview.getByLabel('启用此模块',{exact:true}).isChecked(),false);
  await timer.getByLabel('启用此模块',{exact:true}).uncheck();
  await preview.getByLabel('启用此模块',{exact:true}).check();
  ...
  await page.getByLabel('经验倍率',{exact:true}).selectOption('2');await page.locator('[data-module="experience_scale"]').getByLabel('启用此模块',{exact:true}).check();
  ...
  assert.equal(project.modules.battle_preview.mode,'imperfect');assert.equal(project.modules.battle_preview.enabled,true);assert.equal(project.modules.battle_timer_freeze.enabled,false);assert.equal(project.modules.experience_scale.multiplier,2);assert.equal(project.modules.experience_scale.enabled,true);
